...
ggplot(plastic_waste,
  ...
  labs(
    title = "Plastic waste per capita vs. continent",
...
The continents all seem to have a similar plastic waste per capita value for the majority of their populations. The differences in heights for the peaks of each continent can be explained by the fact that each continent has a different population size. There are slight variances, but overall the continents are much more alike than I expected.
...
We wanted the color and fill to be related to variables, so we included them in the mapping aesthetics. We didn't want the alpha level to be related to a variable, and we wanted it to apply to the whole graph, so we included it in the plotting geom. 
...
The violin plots reveal the shape of the variances in plastic waste per capita for each continent, unlike the box plots. The box plots, however, show the medians and the inner quartile ranges for each continent, which are useful for determining what the majority of the data represents.
...
ggplot(plastic_waste,
  ...
  geom_point()
```
The relationship appears to be that as plastic waste per capita increases, mismanaged waste per capita also increases. The relationship is not perfectly linear, however, the correlation is fairly obvious.
...
ggplot(plastic_waste,
  ...
  geom_point()
```
The correlation does vary between continents. Africa, Asia, and Oceania's dots could be drawn in almost perfectly linear very steep lines, while in Europe and South America there is much less correlation between the two variables. North America is in the middle. Therefore, in Africa, Asia and Oceania, the two variables are associated much more than in Europe and South America, with North America placing right in between.
...
ggplot(plastic_waste,
       aes(x = plastic_waste_per_cap,
           y = total_pop)) +
geom_point(color = "#99CACD")
```
```{r eval=TRUE}
ggplot(plastic_waste,
       aes(x = plastic_waste_per_cap,
           y = coastal_pop)) +
  geom_point(color = "#AB66EF")
```
The two pairs of variables appear to be linearly associated almost exactly the same.
...
plastic_waste %>% 
  ...
  ggplot(aes(x = coastal_pop_prop, y = plastic_waste_per_cap, color = continent)) + 
    ...
    theme_minimal()
```
There appears to be an increase in plastic waste per capita with an increase in coastal population proportion, and then a leveling off that remains constant. This indicates that there is a slight increase in plastic waste as coastal populations increase, and then as they increase further, there is no influence on plastic waste levels. In addition, there is no clear distinction between continents that shows coastal population proportion influencing plastic waste in one continent more than another, other than Africa's coastal population proportion not affecting plastic waste per capita as much compared with the other continents. 
